...
def extract_crash_entry_point(traceback_text, discovered_classes):
    ...
    for file_name, func_name in reversed(matches):
        ...
        for cls in discovered_classes:
            if os.path.basename(cls['file_path']) == base_name and func_name in cls['methods']:
...
Respond with ONLY this identifier. Do not write any explanations, markdown backticks, or punctuation."""
    ...
    cross_class_deps = find_cross_class_dependencies(target_class['file_path'], target_class['code'], method_part, discovered_classes)
    print(f"🔗 Traced cross-class dependencies: {[c['class_name'] for c in cross_class_deps]}")
    ...
    for c_cls in cross_class_deps:
        dep_snippets.append(f"# Cross-Class Dependency: {c_cls['class_name']} (in {c_cls['file_path']})\n{c_cls['code']}")
...
Please provide the corrected python code block inside [PATCH] and [/PATCH] tags, starting directly with 'def method_name(self, ...):'."""
        ...
        if patched_method in target_class['methods']:
            ...
            patch_target_class = next((cls for cls in cross_class_deps if patched_method in cls['methods']), None)
            
        if not patch_target_class:
            # Fallback to scanning all workspace classes
            patch_target_class = next((cls for cls in discovered_classes if patched_method in cls['methods']), None)
            
        if not patch_target_class:
            print(f"❌ Error: Could not resolve target class for patched method '{patched_method}'.")
...
Please provide the corrected method definition inside [PATCH] and [/PATCH] tags with the correct name."""
        ...
        print(f"Applying extracted patch to dynamic target method '{patched_method}' in class '{patch_target_class['class_name']}' ({patch_target_class['file_path']})...")
        start_l, end_l = find_method_line_range(patch_target_class['file_path'], patch_target_class['class_name'], patched_method)
...
Please ensure you are patching the correct method and provide the corrected code block inside [PATCH] and [/PATCH] tags."""
        ...
        if apply_patch_to_file(patch_target_class['file_path'], start_l, end_l, patch_code):
...
Please fix the syntax error and generate a new, correct code block wrapped in [PATCH] and [/PATCH] tags."""
            ...
            entry_file, entry_class, entry_method = extract_crash_entry_point(runtime_stack_trace, discovered_classes)
            if entry_file and entry_class and entry_method:
                print(f"   Using resolved entry point for test run: {entry_class}.{entry_method} in {entry_file}")
                run_success, run_output = verify_runtime_execution(entry_file, entry_class, entry_method)
            else:
