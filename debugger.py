...
Do not put anything else inside the [PATCH] tags except the python function definition."""
    ...
    while current_attempt <= max_retries:
        ...
        if not patch_code:
            ...
            correction_prompt = """You failed to provide the patch code inside the [PATCH] and [/PATCH] tags.
Please provide the corrected python code block inside [PATCH] and [/PATCH] tags."""
        ...
        print(
            f"Applying extracted patch to dynamic target method '{patched_method}' in {target_class['file_path']}..."
        )
        start_l, end_l = find_method_line_range(
            target_class["file_path"], target_class["class_name"], patched_method
        )
...
Please fix the syntax error and generate a new, correct code block wrapped in [PATCH] and [/PATCH] tags."""
            ...
            print(
                "✅ Syntax verification SUCCEEDED. Running dynamic runtime verification..."
            )
